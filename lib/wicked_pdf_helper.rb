module WickedPdfHelper
  ...
  def wicked_pdf_javascript_embeded_tag(sources)
    js_dir = Rails.root.join('public','javascripts')
    sources.collect { |source|
      "<script type='text/javascript'>#{File.read(js_dir.join(source+'.js'))}</script>"
    }.join("\n").html_safe
  end
  module Assets
    def wicked_pdf_stylesheet_link_tag(*sources)
